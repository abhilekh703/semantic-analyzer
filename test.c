...
void expression (struct node *root);
...
int main()
{
    ...
    do
    {
        ...
        switch (ch)
        {
            ...
            case 2: printf("\nEnter the length of expression (including operands, operators and brackets)\n");
			int len,i;
                    scanf("%d", &len);
                    printf("\nTerm        Data Type\n");
                    printf("----------------------\n");
                    ...
                    printf("Float         f\n");
                    printf("double        d\n");
                    printf("(,),operator  n (NULL)\n");
 					printf("\nEnter the expression in following format\nExample:- (6+5.5)-8\n");
 					printf("( n\n6 i\n+ n\n5.5 f\n) n\n- n\n8 i\n");
 					printf("\nEnter your expression now\n");

			for(i=0;i<len;i++)
			{
				scanf("%s %c",&table[i][0],&table[i][1]);
			}
            ...
            case 6: printf("Are you sure you want to QUIT? [1/0]\n");
                    scanf("%d", &choice);
        }
    }while(choice != 1);
...
void expression (struct node *root)
{
	if (root == NULL)
      ...
      if(root->opr==NULL && root->lchild==NULL && root->rchild==NULL)
      {
	      	printf("%c ",root->c);
	      	return;
	  }
     ...
     expression(root->lchild); 	
    
     expression(root->rchild);
}    
